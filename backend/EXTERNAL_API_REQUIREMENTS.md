...
| Facility CEMS data | A data agreement/feed from each plant or SPCB plus a facility-scoped key in `CEMS_INGEST_KEYS_JSON` | The plant operator, CEMS vendor, or relevant SPCB | There is no universal public CEMS feed. Each submitted batch is restricted to one configured facility. |
